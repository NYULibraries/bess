...
<xsl:template name="searchbox_embed">
	...
	<div class="bobcat_embed" id="bobcat_embed_{$unique_key}">
		...
		<div class="bobcat_embed_searchbox" id="bobcat_embed_searchbox_{$unique_key}">
			...
			<form action="{$base_url}/index.php" method="get" name="form1" target="_blank">
				...
				<div class="bobcat_embed_limit_to">
					...
					<select id="input_{./limit_to/media_types/@name}" name="{./limit_to/media_types/@name}">
					...
					<select id="input_{./limit_to/search_types/@name}" name="{./limit_to/search_types/@name}">
					...
					<select id="input_{./limit_to/record_attributes/@name}" name="{./limit_to/record_attributes/@name}">
					...
					<select name="{./limit_to/scopes/@name}">
				            ...
				            <xsl:choose>
				              <xsl:when test="name() = 'pass-vid'">
				                <xsl:choose>
				                  <xsl:when test=". = 'true'">
				                    <xsl:choose>
				                      <xsl:when test="ancestor::view/@vid"><xsl:text>vid</xsl:text>=<xsl:value-of select="ancestor::view/@vid"/></xsl:when>
				                      <xsl:otherwise><xsl:text>vid</xsl:text>=<xsl:value-of select="ancestor::view/@id"/></xsl:otherwise>
			                      </xsl:choose>
			                    </xsl:when>
		...
		</div><!-- /end bobcat_embed_searchbox -->

	</div><!-- /end bobcat_embed -->
